...
window.submitScore = async function (points, level, diamonds) {
    try {
        ...
        // Durasi riil level (detik) — anti-bot, harus cocok dengan validasi server
        const duration = Math.max(1, Math.floor((timestamp - (window._levelStartTime || timestamp)) / 1000));
        const message = `${walletLower}:${points}:${level}:${diamonds}:${duration}:${timestamp}`;
