...
class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        // Player walk animations for the 2 character options
        if (!this.anims.exists('walk_char1')) {
            this.anims.create({
                key: 'walk_char1',
                frames: [
                    { key: 'p1' },
        ...
        if (!this.anims.exists('walk_char2')) {
            this.anims.create({
                key: 'walk_char2',
                frames: [
                    { key: 'play1' },
        ...
        // Determine selected character option
        this.selectedCharInfo = localStorage.getItem('dr_character') || 'char1';
        ...
        const idleTex = this.selectedCharInfo === 'char2' ? 'play1' : 'p1';
        this.player = this.add.sprite(px, py, idleTex);
    ...
    update(time) {
        ...
        if (dx === 0 && dy === 0) {
            // No input — stop walking animation, show idle frame
        ...
        if (!this.playerMoving) {
            ...
            this.player.play('walk_' + this.selectedCharInfo);
        }
